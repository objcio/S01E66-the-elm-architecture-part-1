...
public struct Renderer<A> {
	...
	private func render(label text: String, font: UIFont, into l: UILabel) {
		l.text = text
		l.font = font
	...
	private mutating func render(_ textField: TextField<A>, into result: UITextField) {
        if result.text != textField.text {
            result.text = textField.text
        }
		result.borderStyle = .roundedRect
        result.backgroundColor = textField.backgroundColor
	...
	public mutating func render(_ tableView: TableView<A>, into result: UITableView) {
		if let backing = result.delegate as? TableViewBacking<A> {
	...
	public mutating func render(view: View<A>) -> UIView {
		...
		case let .tableView(tableView):
			...
			return result
		case let .space(width: width, height: height):
			let v = UIView()
...
extension SplitViewController {
	...
	func render(callback: @escaping (Message) -> (), viewController vc: UISplitViewController) -> StrongReferences {
		...
		let detail: ViewController<Message> = ._navigationController(right(vc.viewControllers.count == 1 ? nil : vc.displayModeButtonItem))
...
extension ViewController {
	public func render(callback: @escaping (Message) -> (), change: inout UIViewController) -> StrongReferences {
		switch self {
		case let ._navigationController(newNC):
		...
		case let .viewController(view):
			...
			if change.view.subviews.count == 0 || newView !== change.view.subviews[0] {
				...
				if #available(iOS 11.0, *) {
					newView.topAnchor.constraint(equalTo: change.view.safeAreaLayoutGuide.topAnchor).isActive = true
					newView.bottomAnchor.constraint(lessThanOrEqualTo: change.view.safeAreaLayoutGuide.bottomAnchor).isActive = true
				} else {
					newView.topAnchor.constraint(equalTo: change.topLayoutGuide.bottomAnchor).isActive = true
					newView.bottomAnchor.constraint(lessThanOrEqualTo: change.bottomLayoutGuide.topAnchor).isActive = true
				}
				newView.leftAnchor.constraint(equalTo: change.view.layoutMarginsGuide.leftAnchor).isActive = true
				newView.rightAnchor.constraint(equalTo: change.view.layoutMarginsGuide.rightAnchor).isActive = true
			}
			return r.strongReferences
